...
public class GroupReportGenerator
{
    ...
    {
        this.groupReport = new GroupReport(group);
    }
    ...
    public GroupReport generateGroupReport()
    {
        try
        {
            List<GroupWeeklyReport> groupWeeklyReports = generateGroupWeeklyReports(groupReport.getGroup());
    ...
    private List<GroupWeeklyReport> generateGroupWeeklyReports(Group group) throws IOException
    {
        System.out.println("Processing " + group.getLeaderName() + "'s group.");

        GroupSheetApi groupSheetApi = new GroupSheetApi(group);
        GroupTableData groupTableData = groupSheetApi.getGroupTableData();

        GroupTableDataParser groupTableDataParser = new GroupTableDataParser(groupTableData);

        List<MonthData> coveredMonths = groupTableDataParser.extractMonthsRequiredForReport();

        Pair<Integer, Integer> reportColumns = new ReportColumnsExtractor(groupTableData).getExactColumnsForReportData(coveredMonths);

        ColumnToDateMapper columnToDateMapper = new ColumnToDateMapper();
        columnToDateMapper.initColumnToDateMapFromTableData(coveredMonths, reportColumns, groupTableData.getDatesRow().getValues());

        // get data and parse it
        List<RowData> data = groupSheetApi.getData(reportColumns);

        return generateWeeklyReports(data, group, columnToDateMapper, reportColumns);
    }

    private List<GroupWeeklyReport> generateWeeklyReports(List<RowData> data, Group group, ColumnToDateMapper columnToDateMapper, Pair<Integer, Integer> reportColumns) throws IOException
    {
        ...
        List<ReportItem> reportItems = getItems(people, colors, reportColumns, data, columnToDateMapper);

        List<GroupWeeklyReport> groupWeeklyReports = getWeeks(data, group, columnToDateMapper, colors.get(GROUP));

        groupWeeklyReports = fillWeeks(reportItems, people, groupWeeklyReports);

        handleAddedRemovedToList(groupWeeklyReports, group, people);
